...
import CountUp from 'react-countup'; // Assuming 'Count' is react-countup
import {
  FunnelChart,
  FunnelSeries,
  FunnelArc,
  ...
  FunnelAxisLabel,
  FunnelAxisLine,
  ChartData, // Type for FunnelChart data
} from 'reaviz';
...
const MetricTagIcon: React.FC<{ className?: string; fill?: string }> = ({
  className = 'w-5 h-5',
  fill = '#E84045',
}) => (
  <svg
...
const MetricAlertIcon: React.FC<{ className?: string; fill?: string }> = ({
  className = 'w-5 h-5',
  fill = '#E84045',
}) => (
  <svg
...
const RAW_FUNNEL_DATA: FunnelDataPoint[] = [
  { key: 'Initial Alerts', data: 1250 },
  { key: 'Acknowledged', data: 1120 },
  { key: 'Triaged', data: 950 },
  { key: 'Critical Escalated', data: 321 },
  { key: 'Resolved', data: 280 },
  { key: 'Post-Mortem', data: null as any }, // Intentionally invalid
];

// Validate Funnel Chart Data
RAW_FUNNEL_DATA.forEach((item) => {
  if (typeof item.data !== 'number' || isNaN(item.data)) {
    item.data = 0; // Default to 0 if not a valid number
  }
  if (typeof item.key !== 'string') {
    item.key = String(item.key || 'Unknown Stage');
  }
});
const FUNNEL_CHART_DATA: FunnelDataPoint[] = RAW_FUNNEL_DATA;

const IncidentAnalyticsDashboard: React.FC = () => {
  // For FunnelChart axis colors that adapt to theme, this is a common compromise
  // if the library doesn't easily support dynamic prop changes via CSS or context.
  // A neutral color that works on both light and dark backgrounds.
  const funnelAxisLineColor = '#7E7E8F75'; // Semi-transparent gray
  ...
  return (
    <div className="flex flex-col justify-between pt-4 pb-4 bg-white dark:bg-black rounded-3xl shadow-[11px_21px_3px_rgba(0,0,0,0.06),14px_27px_7px_rgba(0,0,0,0.10),19px_38px_14px_rgba(0,0,0,0.13),27px_54px_27px_rgba(0,0,0,0.16),39px_78px_50px_rgba(0,0,0,0.20),55px_110px_86px_rgba(0,0,0,0.26)] w-[600px] h-[714px] overflow-hidden text-black dark:text-white transition-colors duration-300">
      <div className="flex justify-between items-center p-7 pt-6 pb-8">
        <h3 className="text-3xl text-left font-bold">Incident Report</h3>
        <select
      ...
      <div className="h-[250px] w-full px-4"> {/* Ensure chart has space */}
        <FunnelChart
          id="incidentFunnel"
          height={250}
          ...
          series={
            <FunnelSeries
              arc={
                <FunnelArc
                  colorScheme={['#5B14C5']} // Purple color scheme
                  gradient={null}
                  glow={{
                    blur: 30,
                    color: '#5B14C5', // Glow color matches arc
                  }}
                />
              }
              axis={
                <FunnelAxis
                  label={
                    // Rely on inherited text color for dark/light mode
          ...
          >
            ...
            <div className="flex gap-2 w-1/2 justify-end items-center">
              ...
              >
                ...
                {item.change.replace('-', '').replace('+', '')} {/* Display absolute change */}
              </div>
            </div>
          </motion.div>
        ))}
